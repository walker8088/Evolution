...
class DataStore():
    ...
    def saveEndBook(self, book_name, games):
        ...
        for game in games:
            ...
            ret = self.endbook_table.search((q.book_name == book_name)&(q.name == game['name']))
            if  len(ret) == 0:
    ...
    def changeBookmarkName(self, fen, new_name):
        q = Query()
        ret = self.bookmark_table.update({'bookmark_name' : new_name}, (q.book_fen == fen))
    ...
    def changeMyGameName(self, old_name, new_name):
        ret = self.mygame_table.update({'name' : new_name}, (Query().name == old_name))
        return True
    ...
    def saveMovesToBook(self, positions): 
        ...
        for position in positions:
            ...
            if len(ret) == 0:
                self.position_table.insert({'fen': fen, 'actions':[action_to_save,]})
            elif len(ret) == 1:
                ...
                for act in db_actions:
                    if act['move'] == move_iccs:
                        ...
                        if (action_to_save['score']  == '') and ('score' in act):
                            del action_to_save['score'] 
                        act.update(action_to_save) 
                        self.position_table.update({'actions':db_actions}, q.fen == fen)
