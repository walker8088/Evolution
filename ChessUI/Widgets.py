...
class HistoryWidget(QWidget):
    ...
    def onNextBtnClick(self):
        ...
        if self.selectionIndex < 0 or self.selectionIndex >= (len(self.items) - 1):
            return
...
class DockHistoryWidget(DockWidget):
    def __init__(self, parent):
        super().__init__(parent, Qt.LeftDockWidgetArea | Qt.RightDockWidgetArea)
...
class MoveDbWidget(QDockWidget):
    ...
    def contextMenuEvent(self, event):
        ...
        menu = QMenu(self)        
        importFollowAction = menu.addAction("导入分支(单走)")
        importAllFollowAction = menu.addAction("导入分支(全部)")
        delimportFollowAction = menu.addAction("删除分支")
        action = menu.exec_(self.mapToGlobal(event.pos()))
...
class GameReviewWidget(QDockWidget):
    ...
    def __init__(self, parent):
        ...
        self.moveListView = QTreeWidget()
        self.moveListView.setColumnCount(1)
        self.moveListView.setHeaderLabels(["着法", "红优分",  '备注'])
        self.moveListView.setColumnWidth(0, 80)
...
class BookmarkWidget(QDockWidget):
    ...
    def __init__(self, parent):
        ...
        self.bookmarkTypeCombo.currentTextChanged.connect(self.onBookTypeChanged)
    ...
    def contextMenuEvent(self, event):
        ...
        if action == removeAction:
            ...
            new_name, ok = QInputDialog.getText(self, getTitle(), '请输入新名称:', text = old_name)             
            if ok:
