...
class MainWindow(QMainWindow, QtStyleTools):
    ...
    def __init__(self, app):
        ...
        self.historyView.inner.positionSelSignal.connect(self.onSelectHistoryPosition)
       
        self.endBookView = EndBookWidget(self)
        self.endBookView.setVisible(False)
        self.endBookView.end_game_select_signal.connect(self.onSelectEndGameIndex)
        ...
        self.engineView.showInfoBox.stateChanged.connect(self.onShowInfoBoxChanged)
    ...
    def initGame(self, fen = None, is_only_once = False):
        ...
        self.currPosition = {'fen': self.init_fen, 'index': 0, 'move_side': board.move_player.color}
    ...
    def onEngineBestMove(self, engine_id, move_info):
        ...
        if self.currPosition['index'] > 0:
            if 'score' not in move_info:
                ...
                self.currPosition['score'] =  score if p_color == BLACK  else -score
    ...
    def onMoveGo(self, move_from, move_to, score = ''):
        ...
        position = {'fen': self.board.to_fen(), 
                            'move': move, 'score': score, 'index' : len(self.positionList), 'move_side': move.board.move_player.color}
    ...
    def onOpenFile(self):
        ...
        fileName, _ = QFileDialog.getOpenFileName(self, "打开文件", "","象棋演播室文件(*.xqf);;PGN文件(*.pgn)", options=options)
        ...
        if game:
            game_info = {'fen': game.init_board.to_fen(), 'moves': game.dump_iccs_moves()[0]}
    ...
    def onSaveFile(self):
        ...
        fileName, _ = QFileDialog.getSaveFileName(self, "保存对局文件", "","象棋演播室文件(*.xqf);;PGN文件(*.pgn)", options=options)
    ...
    def onImportEndBook(self):
        ...
        if self.storage.isEndBookExist(lib_name):
            msgbox = TimerMessageBox(f"残局库[{lib_name}]系统中已经存在，不能重复导入。", timeout = 2)
            msgbox.exec()
            return 
    ...
    def createActions(self):
        ...
        self.openFileAct = QAction(self.style().standardIcon(QStyle.SP_FileDialogStart),
                "打开对局", self, statusTip="打开对局文件", triggered=self.onOpenFile)
       
        self.saveFileAct = QAction(self.style().standardIcon(QStyle.SP_DialogSaveButton),
                "保存对局", self, statusTip="保存对局文件", triggered=self.onSaveFile)
    
        self.doOpenBookAct = QAction(QIcon(':Images/openbook.png'),
            "打谱练习", self, statusTip="打谱练习", triggered=self.onDoOpenBook)
        
        self.doEndBookAct = QAction(QIcon(':Images/endbook.png'),
            "残局挑战", self, statusTip="残局挑战", triggered=self.onDoEndBook)
        
        self.doOnlineAct = QAction(QIcon(':Images/online.png'),
            "连线分析", self, statusTip="连线分析", triggered=self.onDoOnline)
        ...
        self.editBoardAct = QAction(QIcon(':Images/edit.png'),
            "自定局面", self, statusTip="从自定局面开始", triggered=self.onEditBoard)
        
        self.searchBoardAct = QAction(QIcon(':Images/search.png'),
            "搜索局面", self, statusTip="从对局库中搜索局面", triggered=self.onSearchBoard)
        
        self.myGamesAct = QAction(QIcon(':Images/mybook.png'), 
            "我的对局库", self, statusTip="我的对局库", triggered=self.onShowMyGames)
        
        self.bookmarkAct = QAction(QIcon(':Images/bookmark.png'),
            "我的收藏", self, statusTip="我的收藏", triggered=self.onShowBookmark)
        
        self.upgradeAct = QAction(
            "升级到专业版", self, statusTip="升级到专业版", triggered=self.onUpgradeToProfessional)
